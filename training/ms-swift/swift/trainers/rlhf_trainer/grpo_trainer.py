...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        self.oversample_multiplier = args.oversample_multiplier
        self.rejection_sample = args.rejection_sample
        self.num_generations = args.num_generations
        
        # Rejection sampling configurations
        self.rejection_strategy = args.rejection_strategy  # "prob" or "tag_tail"
        self.rejection_prob_threshold = args.rejection_prob_threshold  # probability threshold
        self.rejection_prob_consecutive_tokens = args.rejection_prob_consecutive_tokens
        self.rejection_tag_tail = args.rejection_tag_tail
    ...
    def get_train_dataloader(self):
        if self.rejection_sample and self.rejection_strategy:
            ...
            dataloader_params = {
                "batch_size": int(self._train_batch_size * self.args.steps_per_generation * self.oversample_multiplier),  # 为rejection_sampling
    ...
    def _get_train_sampler(self, train_dataset=None):
        ...
        elif self.rejection_sample and self.rejection_strategy:
            # Use oversample multiplier when rejection sampling is enabled with a strategy
            multiplier = self.oversample_multiplier
            return RepeatSampler(
                data_source=train_dataset or self.train_dataset,
                mini_repeat_count=int(self.num_generations * multiplier),
                batch_size=self.args.generation_batch_size // self.num_generations,
    ...
    def _generate_completions(self, inputs: DataType) -> DataType:
        ...
        for i, _input in enumerate(inputs):
            if 'solution' not in _input:
                _input['solution'] = _input['messages'][-1]['content']
        inputs = self._preprocess_inputs(inputs)
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        ...
        # Apply rejection sampling to filter unwanted samples (only during training)
        if self.rejection_sample and self.rejection_strategy and self.model.training:
            inputs = self._apply_rejection_sampling(inputs)
        ...
        with patch_profiling_context(self, 'log_metrics'):
            ...
            # Handle extra outputs returned from reward functions
            if getattr(self, '_extra_outputs_current', None) is not None:
                if 'extra_output' not in self._logs:
                    self._logs['extra_output'] = deque(maxlen=self.args.generation_batch_size)
                self._logs['extra_output'].extend(
                    self._gather_and_flatten(self._extra_outputs_current, flatten_level=0)
                )
                self._extra_outputs_current = None
    ...
    def _compute_rewards_per_func(self, inputs: DataType) -> torch.Tensor:
        ...
        for i, (reward_func, reward_model_plugin, reward_func_name) in enumerate(
            ...
            with patch_profiling_context(self, reward_func_name):
                ...
                # Support (rewards, extra_output) tuple returns
                if isinstance(output_reward_func, tuple) and len(output_reward_func) >= 1:
                    if extra_outputs is None and len(output_reward_func) >= 2:
                        extra_outputs = output_reward_func[1]
                    output_reward_func = output_reward_func[0]
        ...
        # Store extra outputs for logging later in _generate_and_score_completions
        if extra_outputs is not None:
            self._extra_outputs_current = extra_outputs
        else:
            self._extra_outputs_current = None
...
# Rejection Sampling Usage Examples:
#
# 1. Probability-based rejection (reject samples if all tokens in the last N tokens have high probability):
#    args.rejection_sample = True
#    args.rejection_strategy = "prob"
#    args.rejection_prob_threshold = 0.9  # probability threshold
#    args.rejection_prob_consecutive_tokens = 100  # check last N tokens
#
# 2. Tag-tail rejection (reject samples ending with specific tags):
#    args.rejection_sample = True
#    args.rejection_strategy = "tag_tail"
#    args.rejection_tag_tail = "<nl>\n"  # reject samples ending with this string
#
# 3. Combined strategies (both conditions need to be met for rejection):
#    args.rejection_strategy = ["prob", "tag_tail"]
#
# The trainer will automatically oversample by `oversample_multiplier`
# and then filter out rejected samples to maintain the desired `num_generations` count.

    def _should_reject_sample(self, completion_text: str, token_logprobs: Optional[List[float]] = None) -> bool:
        """
        Determine if a sample should be rejected based on the configured rejection strategy.
        
        Args:
            completion_text: The generated completion text
            token_logprobs: Optional list of token log probabilities
            
        Returns:
            bool: True if the sample should be rejected, False otherwise
        """
        if not self.rejection_sample or not self.rejection_strategy:
            return False

        should_reject = False
        if "prob" in self.rejection_strategy:
            should_reject = should_reject or self._should_reject_by_prob(token_logprobs)
        if "tag_tail" in self.rejection_strategy:
            should_reject = should_reject or self._should_reject_by_tag_tail(completion_text)
        return should_reject
    
    def _should_reject_by_prob(self, token_logprobs: Optional[List[float]]) -> bool:
        """
        Check if sample should be rejected based on high probability tokens in the last N tokens.
        
        Args:
            token_logprobs: List of token log probabilities (from _get_per_token_logps_and_entropies)
            
        Returns:
            bool: True if all tokens in the last N tokens exceed probability threshold
        """
        if not token_logprobs:
            return False
            
        # Get the last N tokens (or all tokens if fewer than N)
        last_n_tokens = min(self.rejection_prob_consecutive_tokens, len(token_logprobs))
        last_token_logprobs = token_logprobs[-last_n_tokens:]
        
        # Convert log probabilities to probabilities for the last N tokens
        # token_logprobs from _get_per_token_logps_and_entropies are log probabilities
        probs = []
        for logprob in last_token_logprobs:
            if logprob is not None and not torch.isnan(torch.tensor(logprob)):
        ...
        # Check if all tokens in the last N tokens exceed the threshold
        if len(probs) < last_n_tokens:
            return False
            
        return all(prob > self.rejection_prob_threshold for prob in probs)
    
    def _should_reject_by_tag_tail(self, completion_text: str) -> bool:
        """
        Check if sample should be rejected based on tail tag.
        
        Args:
            completion_text: The generated completion text
            
        Returns:
            bool: True if text ends with rejection tag
        """
        return not completion_text.strip().endswith(self.rejection_tag_tail.strip())

    @patch_profiling_decorator
    def _apply_rejection_sampling(self, inputs: DataType) -> DataType:
        """
        Apply rejection sampling to filter out unwanted samples and maintain desired count.
        严格保持原始输入的连续顺序结构，以确保与rewards.view(-1, self.num_generations)的兼容性。
        
        Args:
            inputs: List of input samples with completions
            
        Returns:
            DataType: Filtered inputs maintaining exact sequential structure for num_generations grouping
        """
        if not self.rejection_sample or not self.rejection_strategy:
            return inputs
        ...
        oversample_size = int(self.num_generations * self.oversample_multiplier)
        num_groups = len(inputs) // oversample_size
        
        for group_idx in range(num_groups):
            start_idx = group_idx * oversample_size
            end_idx = start_idx + oversample_size
            group_inputs = inputs[start_idx:end_idx]
            
            # Filter valid samples within this group
            valid_samples = []
            group_total = len(group_inputs)
            total_samples += group_total
            
            for inp in group_inputs:
                completion = inp['messages'][-1]['content']
                
                # Get token logprobs for probability-based rejection
                token_logprobs = None
                if self.rejection_strategy == "prob":
                    try:
                        # Encode the input to get the format needed for _get_per_token_logps_and_entropies
                        template = self.template
                        with self._template_context(template):
                            encoded_inp = template.encode(inp)
                            # Convert to tensor format and move to device
                            batch_encoded = template.data_collator([encoded_inp])
                            batch_encoded = {k: v.to(self.model.device) if isinstance(v, torch.Tensor) else v 
                                           for k, v in batch_encoded.items()}
                            
                            # Get token logprobs using the existing method
                            ...
                            try:
                                with torch.no_grad():
                                    per_token_logps, _ = self._get_per_token_logps_and_entropies(
                                        self.model, batch_encoded, compute_entropy=False)
                                    if per_token_logps is not None:
                                        token_logprobs = per_token_logps.flatten().cpu().tolist()
                            finally:
                                # Restore original training mode
                                if was_training:
                                    self.model.train()
                    except Exception as e:
                        logger.warning(f"Failed to get token logprobs for rejection sampling: {e}")
                        token_logprobs = None
                
                # Check if sample should be rejected
                should_reject = self._should_reject_sample(completion, token_logprobs)
                if should_reject:
                    rejected_samples += 1
                else:
                    valid_samples.append(inp)
            
            if len(valid_samples) >= self.num_generations:
                group_result = valid_samples[:self.num_generations]
            else:
                group_result = valid_samples[:]
                remaining_needed = self.num_generations - len(group_result)
                if remaining_needed > 0:
                    # 从原组中按顺序补充样本
                    for inp in group_inputs:
                        if len(group_result) >= self.num_generations:
                            break
                        if inp not in group_result:
                            group_result.append(inp)
            
            filtered_inputs.extend(group_result[:self.num_generations])
        
        remaining_start = num_groups * oversample_size
        if remaining_start < len(inputs):
            remaining_inputs = inputs[remaining_start:]
            valid_remaining = []
            for inp in remaining_inputs:
                ...
                if not should_reject:
                    valid_remaining.append(inp)
                else:
                    rejected_samples += 1
            
            filtered_inputs.extend(valid_remaining if valid_remaining else remaining_inputs)
            total_samples += len(remaining_inputs)
        
        # Log rejection sampling metrics
        if total_samples > 0:
            mode = 'train' if self.model.training else 'eval'
            
            # Gather metrics from all ranks to get global statistics
            global_total_samples = self._gather_and_flatten([total_samples], dtype=torch.float32, device=self.accelerator.device, flatten_level=0).sum().item()
            global_rejected_samples = self._gather_and_flatten([rejected_samples], dtype=torch.float32, device=self.accelerator.device, flatten_level=0).sum().item()
            global_rejection_rate = global_rejected_samples / global_total_samples if global_total_samples > 0 else 0.0
            
            self._metrics[mode][f'rejection_sampling/rejection_rate'].append(global_rejection_rate)
            self._metrics[mode][f'rejection_sampling/total_samples'].append(global_total_samples)
            self._metrics[mode][f'rejection_sampling/rejected_samples'].append(global_rejected_samples)
            
            # Summary filter results warning
            final_samples = len(filtered_inputs)
            logger.warning(
                f"Rejection Sampling Summary - Strategy: {self.rejection_strategy} | "
                f"Local: Total: {total_samples} -> Rejected: {rejected_samples} -> Final: {final_samples} | "
                f"Global: Total: {global_total_samples:.0f} -> Rejected: {global_rejected_samples:.0f} ({global_rejection_rate:.2%}) | "
                f"Expected: {num_groups * self.num_generations} | Groups: {num_groups} | "
                f"Oversample Size: {oversample_size} | Multiplier: {self.oversample_multiplier}x"
            )
